...
from dataset.dataset_F import PileDataset
...
# general args
# Dataset
parser.add_argument('--dataset_name', dest='dataset_name', default='Pile_F', type=str)
parser.add_argument('--data_num', dest='data_num', default=400, type=int)
...
model.eval()
model.load_state_dict(torch.load('checkpoints/Pile_F/model_1.pth', map_location=device))
# Save result path
task = args.dataset_name
save_model_dir = 'results/pilegraph/'
if not os.path.exists(save_model_dir):
    os.makedirs(save_model_dir)

for data,filename,_ in tqdm(test_loader):
    data = data.to(device)
