...
class PileDataset(Dataset):
    ...
    def __init__(self, split,txt_path = 'data/data_F'):
...
def get_trans_mc(txt_lines,source_lines):
    ...
    if txt5[0] == 1.2:
        ...
        D = -0.3
        P = -0.3
    K = normalize_parameter(txt7[0],1.0,1.5)
    ...
    for z in range(len(txt1)):
        ...
        edge_attr[z, 3] = 0.9 * normalize_parameter(txt3[z], 0, 350)  # qik
...
def get_trans_source(source_lines,edge_num):
    ...
    node_list = [normalize_parameter(node, 0, 70)for node in node_list]
